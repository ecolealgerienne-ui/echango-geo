...
geo_get "/v1/geocode/search?q=paris&country=fr"
assert_eq "400" "$GEO_HTTP" "country=fr → 400 (et non 200 à résultat vide)"
assert_eq "geo.country_not_configured" "$(jget '.code')" "code geo.country_not_configured"

# ── Un des pays d'une liste est non supporté → toute la requête est refusée ─
geo_get "/v1/geocode/search?q=souk&country=dz,fr"
...
geo_get "/v1/geocode/search?q=souk&country=FRANCE"
assert_eq "400" "$GEO_HTTP" "country=FRANCE → 400"
assert_eq "geo.invalid_query" "$(jget '.code')" "forme invalide distinguée du pays non configuré"
